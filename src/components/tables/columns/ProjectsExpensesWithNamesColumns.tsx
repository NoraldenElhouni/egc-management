...
import { ProjectExpenseWithName } from "../../../types/extended.type";
...
export const ProjectsExpensesWithNamesColumns: ColumnDef<ProjectExpenseWithName>[] =
  [
    // SELECT CHECKBOX
    {
      id: "select",
    ...
    {
      accessorKey: "name",
      accessorFn: (row) => row.projects?.name || "N/A",
      header: "الاسم المشروع",
      cell: ({ row }) => (
        <Link
          to={`/finance/bookkeeping/project/${row.original.project_id}`}
          className="font-medium underline max-w-[200px] truncate block"
