...
const EditRoundForm = ({
  ...
  const { editRound, loading } = useEditRound();
  const [showPicker, setShowPicker] = useState(false);
  const [files, setFiles] = useState<AttachmentDraft[]>([]);
  const [confirmOpen, setConfirmOpen] = useState(false);

  const defaultItems: RoundForm["items"] = existingRound.round_items.map(
    (i) => ({
      work_id: i.work_id,
  ...
  const {
    ...
    defaultValues: {
      specialization_id: existingRound.specialization_id,
  ...
  const onSubmit = async (data: RoundForm) => {
    const { error } = await editRound(data, roundId);
    if (error) {
      alert("خطأ في تعديل جولة التسعير: " + error.message);
      return;
    }
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto p-6 bg-white rounded-lg shadow-sm">
        <h1 className="text-2xl font-semibold mb-6">تعديل جولة التسعير</h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <ConfirmDialog
            ...
            title="حفظ التعديلات"
            message="هل أنت متأكد من حفظ التعديلات على هذه الجولة؟"
