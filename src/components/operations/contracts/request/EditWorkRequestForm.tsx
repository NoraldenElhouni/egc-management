...
import { Service } from "../../../../hooks/operations/contracts/useContracts";
...
const EditWorkRequestForm = ({
  ...
  const {
    ...
    defaultValues: {
      specialization_id: existingRequest.specialization_id,
      title: existingRequest.title,
      description: existingRequest.description,
      bid_deadline: existingRequest.bid_deadline?.slice(0, 10) ?? "",
      work_start_at: existingRequest.work_start_at?.slice(0, 10) ?? "",
      bid_mode: existingRequest.mode,
      direct_contractor_id: existingRequest.direct_contractor_id ?? undefined,
      contact_name: existingRequest.contact_name ?? "",
      contact_phone: existingRequest.contact_phone ?? "",
      delay_penalty_terms: existingRequest.delay_penalty_terms ?? "",
      retention_terms: existingRequest.retention_terms ?? "",
  ...
  const onSubmit = async (data: RequestForm) => {
    const { error } = await editRequest(data, requestId);
    if (error) {
      alert("خطأ في تعديل طلب العقد: " + error.message);
      return;
    }
    setSuccess("تم تعديل طلب العقد بنجاح");
    navigate(-1);
  };
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto p-6 bg-white rounded-lg shadow-sm">
        <h1 className="text-2xl font-semibold mb-6">تعديل طلب العقد</h1>
...
export default EditWorkRequestForm;
